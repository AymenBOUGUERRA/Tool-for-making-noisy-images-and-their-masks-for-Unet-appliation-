...
def maino(img, grid, filename, i):
    ...
    if img_shape[0] >= grid_shape[0] or img_shape[1] >= grid_shape[1]:
        img = cv2.resize(img, (int(img_shape[1]*0.6), int(img_shape[0]*0.6)))
    img_shape = img.shape
    ...
    cv2.imwrite('/noisy/' + str(i) + ".png", blackAndWhiteImage)
    cv2.imwrite('/clean/' + str(i) + ".png", blackAndWhiteImage_original)
    return print("Done")
...
for filename in os.listdir(directory):
    ...
    if rand == 2:
        grid = cv2.imread('/french_grid.png',
                          cv2.IMREAD_GRAYSCALE)
        img = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
        maino(img, grid, filename, i)
    if rand == 3:
        grid = cv2.imread('/lines_grid.png',
                          cv2.IMREAD_GRAYSCALE)
        img = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
        maino(img, grid, filename, i)
    if rand == 4:
        grid = cv2.imread('/standard_grid.png',
                          cv2.IMREAD_GRAYSCALE)
        img = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
        maino(img, grid, filename, i)
    if rand == 5:
        grid = cv2.imread('/no_grid.png',
                          cv2.IMREAD_GRAYSCALE)
        img = cv2.imread(f, cv2.IMREAD_GRAYSCALE)
        maino(img, grid, filename, i)
    i = i + 1
